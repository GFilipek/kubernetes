...
type ContainerPolicy struct {
	...
	ResourcePolicy map[api.ResourceName]Policy
}
...
type RecommenderAPI interface {
	...
	GetRecommendation(spec *api.PodSpec) (*Recommendation, error)
}
...
func (rec *recommenderAPIImpl) GetRecommendation(spec *api.PodSpec) (*Recommendation, error) {
	...
	for i, podContainer := range spec.Containers {
		...
		result[i] = ContainerRecommendation{
			...
			Resources: map[api.ResourceName]resource.Quantity{
				api.ResourceCPU: cpu, api.ResourceMemory: memory}}
	}
	...
	return &VerticalPodAutoscaler{
		Spec: Spec{
			...
			ResourcesPolicy: ResourcesPolicy{Containers: []ContainerPolicy{{
				Name: "slave",
				ResourcePolicy: map[api.ResourceName]Policy{
					api.ResourceCPU:    {minCpu, maxCpu},
					api.ResourceMemory: {minMem, maxMem}},
			}}}},
	}
}
